...
#         # future         = fcast[pd.to_datetime(fcast["timestamp"]) >= pd.Timestamp.now()]
...
#         future         = fcast[pd.to_datetime(fcast["timestamp"]) >= tomorrow_start]
#         daily          = future.groupby("date")["predicted_aqi"].mean().reset_index().head(3)
#         if len(daily) > 0:
...
#                     {pd.Timestamp(row["date"]).strftime("%A").upper()}<br>
#                     {pd.Timestamp(row["date"]).strftime("%d %B")}
#                   </div>
